CREATE FUNCTION dbo.fn_Get_Cron_Hours 
(
    @STRING VARCHAR (255)
) 
RETURNS TABLE
...
(
    SELECT 
        [value]     = CASE WHEN [value] = '*' THEN '0-23/1' ELSE [value] END,
        [type]      = CASE 
                                ...
                                SELECT 
                                    ...
                                    VAL_INTERVAL    = ISNULL(
                                                        CASE 
                                                            ...
                                                            WHEN (CONVERT(TINYINT, P.[2]) % 24) = 0 THEN 1
                                                            /*Cover all bases, if we go higher then 23, mod will over the issue*/
                                                            ELSE CONVERT(TINYINT, P.[2]) % 24
                                                        END, 1)
                                FROM 
                                (
                                    /*
...
SELECT DISTINCT 
    COALESCE(S.[value], P.[value]) AS [Hours]
FROM 
    PARSE_VALUES AS S 
